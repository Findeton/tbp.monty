...
"""Panda3D evaluation harness for CorticalColumnTorch (modern Hopfield).

Mirrors :class:`CorticalColumnEvalHarness` but uses the PyTorch-based
:class:`CorticalColumnTorch` with modern Hopfield dynamics. Allows direct
accuracy/speed comparison against the SDR CorticalColumn and EvidenceGraphLM.
...
Usage::
    ...
    harness = CorticalColumnTorchEvalHarness(object_names=YCB_EVAL_OBJECTS[:5])
    result = harness.run()
...
class CorticalColumnTorchEvalHarness:
    """Object recognition evaluation using CorticalColumnTorch + Panda3D.
    ...
    train_episodes : int
        Number of training passes per object.
    ...
    column_kwargs : dict or None
        Keyword arguments for ``CorticalColumnTorch``.
    ...
    def __init__(
        ...
        self._column: CorticalColumnTorch | None = None
    ...
    def _setup(self) -> None:
        ...
        self._column = CorticalColumnTorch(**col_kw)
    ...
    def run(self) -> EvalRunResult:
        ...
        config = {
            "architecture": "CorticalColumnTorch (Modern Hopfield)",
            ...
            "n_hopfield_patterns": self._column._hopfield.n_stored,
            "n_dendrite_segments": self._column._dendrites.total_segments,
        }
    ...
    def get_column(self) -> CorticalColumnTorch:
